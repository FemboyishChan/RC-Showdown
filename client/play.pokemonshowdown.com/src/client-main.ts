...
class PSUser extends PSStreamModel<PSLoginState | null> {
	...
	changeName(name: string) {
		...
		this.loggingIn = name;
		this.update(null);
		const loginChallstr = this.challstr;
		PSLoginServer.rawQuery(
			'getassertion', { userid, challstr: this.challstr }
		).then(res => {
			// If the connection reset during login, retry with new challstr (max 1 retry)
			if (this.challstr !== loginChallstr && res && !res.startsWith(';')) {
				this.loggingIn = null;
				if (!this._renameRetries) {
					this._renameRetries = 1;
					this.changeName(name);
					return;
				}
			}
			this._renameRetries = 0;
	...
	changeNameWithPassword(name: string, password: string, special: PSLoginState = { needsPassword: true }) {
		...
		PSLoginServer.query(
			...
			// If the connection reset during login, retry with new challstr (max 1 retry)
			if (this.challstr !== loginChallstr) {
				this.loggingIn = null;
				if (!this._loginRetries) {
					this._loginRetries = 1;
					this.changeNameWithPassword(name, password, special);
					return;
				}
			}
...
class PSServer {
	...
	prefix = '/showdown';
	protocol: 'http' | 'https' = Config.defaultserver.protocol as 'http' | 'https' || (Config.defaultserver.httpport ? 'https' : 'http');
