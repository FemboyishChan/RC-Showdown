(function ($) {
	...
	this.Topbar = Backbone.View.extend({
		...
		initialize: function () {
			...
			this.$el.html('<img class="logo" src="' + Dex.rootPrefix + 'RC_Logo.png" srcset="' + Dex.rootPrefix + 'RC_Logo.png 2x" alt="Pok&eacute;mon Showdown! (beta)" width="110" height="60" /><div class="maintabbarbottom"></div><div class="tabbar maintabbar"><div class="inner"></div></div><div class="userbar"></div>');
			...
			if (window.nodewebkit) {
				...
				if (!!$favicon.data('on') !== !!notificationCount) {
					if (notificationCount) {
						$favicon.attr('href', Dex.rootPrefix + '/favicon-notify.ico');
						$favicon.data('on', '1');
					} else {
						$favicon.attr('href', Dex.rootPrefix + '/favicon.ico');
						$favicon.data('on', '');
					}
				}
			}
		...
		updateTabbarMini: function () {
			...
			var buf = '<ul><li><a class="button minilogo' + notificationClass + '" href="' + app.root + '"><img src="' + Dex.resourcePrefix + 'RC_favicon512.png" width="32" height="32" alt="Pok&eacute;mon Showdown! (beta)" /><i class="fa fa-caret-down" style="display:inline-block"></i></a></li></ul>';
	...
	var CustomBackgroundPopup = this.CustomBackgroundPopup = Popup.extend({
		...
		initialize: function () {
			...
			buf += '<p><strong>Background</strong></p>';
			buf += '<div class="bglist">';

			buf += '<button name="setBg" value="main_game_logo" class="option' + (cur === 'main_game_logo' || !cur ? ' cur' : '') + '"><img src="fx/client-bg-main_game_logo.png" style="width:80px;height:80px;object-fit:cover;display:block">Default</button>';
		...
		setBg: function (bgid) {
			var bgUrl = (bgid === 'solidblue' ? '#344b6c' : bgid === 'main_game_logo' ? 'fx/client-bg-main_game_logo.png' : Dex.resourcePrefix + 'fx/client-bg-' + bgid + '.jpg');
	...
	var RegisterPopup = this.RegisterPopup = Popup.extend({
		...
		submit: function (data) {
			...
			// attempt to talk to a real login server first.  if the request fails
			// (for example because you're running the client off the file system
			// and don't have a server running), fall back to a trivial
			// localStorage-based "database" so the register button still does
			// something.
			var ajax = $.post(app.user.getActionPHP(), postData, Storage.safeJSON(function (data) {
				if (!data) data = {};
			...
			ajax.fail(function () {
				// network failure; register locally
				...
				if (users[userid]) {
					app.addPopup(RegisterPopup, {
						name: name,
						captcha: captcha,
						error: 'Username already taken (local)'
					});
					return;
				}
				users[userid] = {
				...
				localStorage.setItem('showdown-localusers', JSON.stringify(users));
				app.user.set('registered', {userid: userid, username: name});
				app.addPopupMessage("Registered locally (no login server detected).");
			});
		}
	});
