...
(function ($) {
	...
	Config.root = '/';
	// when developing with the built-in login/replay server on localhost:3000,
	// make replay links point back here. this ensures the "Upload and share
	// replay" button actually produces a working URL.
	if (window.location.hostname === 'localhost' && window.location.port === '3000') {
		Config.routes = Config.routes || {};
		Config.routes.replays = window.location.host + '/replay';
	}
	...
	var User = this.User = Backbone.Model.extend({
		...
		getActionPHP: function () {
			// For local development, always send login requests to the local
			// login server on port 3000.  We return a full http:// URL so that
			// the testclient $.post override (which rewrites relative "/" URIs
			// to play.pokemonshowdown.com) leaves it alone.
		...
		rename: function (name) {
			...
			// Local mode: skip login server authentication and just rename directly
			// This works when the server has noguestsecurity enabled
			if (Config.localmode) {
				app.send('/trn ' + name);
				return;
			}
			...
			if (this.get('userid') !== userid) {
				var self = this;
				var loginChallstr = this.challstr;
				$.post(this.getActionPHP(), {
					act: 'getassertion',
					userid: userid,
					challstr: this.challstr
				}, function (data) {
					// If connection reset during login, retry with new challstr (max 1 retry)
					if (self.challstr !== loginChallstr && data && data.charAt(0) !== ';') {
						if (!self._renameRetries) {
							self._renameRetries = 1;
							self.rename(name);
							return;
						}
					}
		...
		passwordRename: function (name, password, special) {
			var self = this;
			var loginChallstr = this.challstr;
			console.log('[CHALLSTR-DEBUG] passwordRename sending challstr: length=' + this.challstr.length + ' value=' + this.challstr.substring(0, 80) + '...');
			var ajax = $.post(this.getActionPHP(), {
				act: 'login',
				name: name,
				pass: password,
				challstr: this.challstr
				...
				// If connection reset during login, retry with new challstr (max 1 retry)
				if (self.challstr !== loginChallstr) {
					if (!self._loginRetries) {
						self._loginRetries = 1;
						self.passwordRename(name, password, special);
						return;
					}
				}
				self._loginRetries = 0;
			...
			ajax.fail(function () {
				// no login server available; look for credentials in the local
				// registration storage
				var userid = toUserid(name);
				var users = JSON.parse(localStorage.getItem('showdown-localusers') || '{}');
				if (users[userid] && users[userid].password === password) {
					self.set('registered', {userid: userid, username: name});
					self.finishRename(name, '');
					return;
				}
				app.addPopup(LoginPasswordPopup, {
					username: name,
					error: 'Wrong password (local)',
					special: special
				});
			});
		},
		...
		receiveChallstr: function (challstr) {
			if (challstr) {
				...
				console.log('[CHALLSTR-DEBUG] receiveChallstr: length=' + challstr.length + ' value=' + challstr.substring(0, 80) + '...');

				// Local mode: skip login server authentication
				// Users can rename themselves using the Choose Name button
				if (Config.localmode) {
					this.loaded = true;
	...
	this.App = Backbone.Router.extend({
		...
		receive: function (data) {
			...
			case 'challstr':
				// Debug: log the raw data and parts to trace extra pipe-separated segments
				console.log('[CHALLSTR-DEBUG] raw data length=' + data.length);
				console.log('[CHALLSTR-DEBUG] parts.length=' + parts.length + ', parts[1].length=' + (parts[1] || '').length + ', parts[2].length=' + (parts[2] || '').length);
				if (parts.length > 3) {
					console.log('[CHALLSTR-DEBUG] WARNING: parts has ' + parts.length + ' elements (expected 3). Extra parts:');
					for (var pi = 3; pi < Math.min(parts.length, 8); pi++) {
						console.log('[CHALLSTR-DEBUG]   parts[' + pi + ']: length=' + parts[pi].length + ' value="' + parts[pi].substring(0, 60) + '"');
					}
				}
				if (parts[2]) {
		...
		uploadReplay: function (data) {
			...
			var post = $.post(app.user.getActionPHP(), {
				act: 'uploadreplay',
				log: data.log,
				serverid: serverid,
			...
			post.fail(function () {
				if (silent) return;
				// network/server failure – fall back to a data URI that the user can copy
				var room = app.rooms['battle-' + data.id] || app.curRoom;
				var href;
				try {
					href = BattleLog.createReplayFileHref(room);
				} catch (e) {
					href = '';
				}
				if (href) {
					app.addPopup(ReplayUploadedPopup, { url: href });
				} else {
					app.addPopupMessage("Unable to create replay link.");
				}
			});
		},
		...
		updateTitle: function (room) {
			document.title = room.title ? room.title + " - RC Showdown!" : "RC Showdown!";
		},
	...
	var ReplayUploadedPopup = this.ReplayUploadedPopup = Popup.extend({
		...
		initialize: function (data) {
			var buf = '';
			buf = '<p>Your replay' + (data.url ? ' is ready:' : " has been uploaded! It's available at:") + '</p>';
			var link = data.url || ('https://' + Config.routes.replays + '/' + data.id);
			buf += '<p> <a class="replay-link" href="' + link + '" target="_blank" class="no-panel-intercept">' + link + '</a> <button name="copyReplayLink" class="button">Copy</button></p>';
