...
function runMovesearch(target: string, cmd: string, message: string, isTest: boolean) {
	...
	for (const moveid of validMoves) {
		...
		if (move.gen <= mod.gen) {
			if (
				(!nationalSearch && move.isNonstandard && !["Gigantamax", "Custom"].includes(move.isNonstandard)) ||
				(nationalSearch && move.isNonstandard && !["Gigantamax", "Past", "Unobtainable", "Custom"].includes(move.isNonstandard)) ||
