...
export class TeamValidator {
	...
	checkAbility(set: PokemonSet, ability: Ability, setHas: { [k: string]: true }) {
		...
		if (banReason) {
			// Power Construct is an intrinsic event ability for Zygarde formes;
			// exempt it from format ability bans so Zygarde-50%/10% can always use it.
			const zygardeFormes = ['zygarde', 'zygarde10'];
			const isZygardePowerConstruct = ability.id === 'powerconstruct' &&
				zygardeFormes.includes(this.toID(set.species));
			if (!isZygardePowerConstruct) {
