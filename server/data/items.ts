export const Items: import('../sim/dex-items').ItemDataTable = {
	...
	zygardite: {
		...
		onAfterMega(pokemon) {
			if (pokemon.species.id !== 'zygardemega') return;
			const nlMove = this.dex.moves.get('nihillight');
			const newMaxPP = nlMove.noPPBoosts ? nlMove.pp : Math.floor(nlMove.pp * 8 / 5);
			for (const slot of pokemon.moveSlots) {
				if (slot.id === 'coreenforcer') {
					slot.id = 'nihillight' as ID;
					slot.move = 'Nihil Light';
					slot.pp = newMaxPP;
					slot.maxpp = newMaxPP;
					this.add('-message', `${pokemon.name}'s Core Enforcer transformed into Nihil Light!`);
					break;
				}
			}
		},
