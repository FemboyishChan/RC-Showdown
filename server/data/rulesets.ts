...
export const Rulesets: import('../sim/dex-formats').FormatDataTable = {
	...
	alphabetsameletter: {
		effectType: 'ValidatorRule',
		name: 'Alphabet Same Letter',
		desc: "All Pok\u00E9mon on your team must have names starting with the same letter.",
		onValidateTeam(team) {
			...
			for (const set of team) {
				...
				if (!letter) {
					...
					return [
						`All Pok\u00E9mon on your team must start with the same letter ` +
						`(team letter: "${letter}", but ${species.name} starts with "${firstLetter}").`,
					];
				}
			}
		},
	},
	limittwospecialcategory: {
		effectType: 'ValidatorRule',
		name: 'Limit Two Special Category',
		desc: "Limit two Pok\u00E9mon that are Sub-Legendary, Ultra Beasts, or Paradox Pok\u00E9mon.",
		onValidateTeam(team) {
			const special = [];
			for (const set of team) {
				const species = this.dex.species.get(set.species);
				if (
					...
					special.push(species.name);
				}
			}
			if (special.length > 2) {
				return [
					`You can only use up to 2 Sub-Legendary, Ultra Beast, or Paradox Pok\u00E9mon ` +
					`(you have: ${special.join(', ')}).`,
				];
			}
		},
	},
