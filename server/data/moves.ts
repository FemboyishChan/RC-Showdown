...
export const Moves: import('../sim/dex-moves').MoveDataTable = {
	...
	eggbomb: {
		...
		flags: { protect: 1, mirror: 1, metronome: 1, bullet: 1 },
		onModifyMove(move, pokemon, target) {
			if (target && target.side === pokemon.side && target !== pokemon) {
				move.basePower = 0;
				move.heal = [1, 2];
			}
		},
	...
	focuspunch: {
		...
		condition: {
			...
			onHit(pokemon, source, move) {
				if (move.category !== 'Status' && !pokemon.hasAbility('innerfocus')) {
			...
			if (pokemon.hp) {
				...
				for (const condition of sideConditions) {
					if (pokemon.side.removeSideCondition(condition)) {
						this.add('-sideend', pokemon.side, this.dex.conditions.get(condition).name, '[from] move: Rubble Rocket', `[of] ${pokemon}`);
					}
					if (target.side.removeSideCondition(condition)) {
						this.add('-sideend', target.side, this.dex.conditions.get(condition).name, '[from] move: Rubble Rocket', `[of] ${pokemon}`);
					}
				}
			}
		},
	...
	spikes: {
		...
		condition: {
			...
			onSwitchIn(pokemon) {
				...
				if (pokemon.hasAbility && pokemon.hasAbility('tangledfeet')) return;
