		...
		try {
			...
			const staticRequestHandler = (req: http.IncomingMessage, res: http.ServerResponse) => {
				...
				req.addListener('end', () => {
					...
					// Default to serving client files
					...
					if (req.url) {
						if (req.url === '/custom.css' || req.url.startsWith('/custom.css?')) {
							...
							// Serve client config files (testclient-key.js, config.js, etc.)
							req.url = req.url.slice(7);
							server = clientConfigServer;
						} else if (req.url === '/' || req.url === '') {
							// Serve testclient.html as the main page
							req.url = '/testclient.html';
							server = clientServer;
						} else if (roomidRegex.test(req.url)) {
							// Room URLs should serve the client
							req.url = '/testclient.html';
							server = clientServer;
						}
					}
			...
			console.log('Could not start static server:', e);
		}
