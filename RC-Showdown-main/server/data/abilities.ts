...
export const Abilities: import('../sim/dex-abilities').AbilityDataTable = {
	...
	angerpoint: {
		onHit(target, source, move) {
			...
			const state: any = pokemon as any;
			if (state.angerStacks === undefined) state.angerStacks = 0;
			if (state.angerStacks >= 5) return;
			state.angerStacks++;
			this.add('-activate', pokemon, 'ability: Anger Point', String(state.angerStacks));
		},
		onBasePowerPriority: 21,
		onBasePower(basePower, attacker, defender, move) {
			if (!attacker) return;
			if (attacker.ability !== 'angerpoint') return;
			const stacks = (attacker as any)?.angerStacks || 0;
			if (!stacks) return;
			const mult = 1 + 0.1 * stacks;
			const num = Math.round(mult * 4096);
			this.debug(`Anger Point boost: ${mult}`);
			return this.chainModify([num, 4096]);
		},
		flags: {},
		name: "Anger Point",
		rating: 1,
	...
	anticipation: {
		onStart(pokemon) {
			...
			if (hasDangerousMove) {
				pokemon.addVolatile('anticipationdamager');
			}
		},
		flags: {},
		name: "Anticipation",
	...
	anticipationdamager: {
		...
		onAfterMove(move, pokemon) {
			pokemon.removeVolatile('anticipationdamager');
		},
		name: "Anticipation",
		rating: 0,
		num: 0,
	...
	emergencyexit: {
		onDamagingHit(damage, target, source, move) {
			if (damage >= target.baseMaxhp / 4) {
				this.effectState.triggered = true;
			}
		},
		onResidualPriority: -7,
		onResidual(pokemon) {
	...
	moley: {
		...
		onStart(pokemon) {
			...
			this.effectState.entry = true;
		},
		onDamagingHit(damage, target, source, move) {
			if (this.effectState.entry && ['Stealth Rock'].includes(move.name)) {
				return false;
			}
		},
		onTryHit(target, source, move) {
	...
	overgrow: {
		onModifyAtkPriority: 5,
		onModifyAtk(atk, attacker, defender, move) {
			if (move.type === 'Grass' && !this.field.isTerrain('grassy')) {
	...
	revup: {
		...
		rating: 4,
		num: 310,
		shortDesc: "At the end of each turn, raises this Pokemon's Speed by 1.",
		desc: "At the end of each turn, this Pokemon's Speed is raised by 1 stage.",
	},
	ripen: {
	...
	rockypayload: {
		...
		onEffectiveness(typeMod, target, type, move) {
			if (move && move.id === 'stealthrock') return 0;
		},
		flags: {},
		name: "Rocky Payload",
		rating: 3.5,
	...
	slowstart: {
		onStart(pokemon) {
			if (pokemon.m.slowStartTurns === undefined) {
				pokemon.m.slowStartTurns = 5;
			}
			if (pokemon.m.slowStartTurns > 0) {
		...
		onResidualSubOrder: 2,
		onResidual(pokemon) {
			if (pokemon.activeTurns && pokemon.m.slowStartTurns > 0) {
				pokemon.m.slowStartTurns--;
				if (pokemon.m.slowStartTurns === 0) {
	...
	solarpower: {
		...
		onWeather(target, source, effect) {
			...
			if (effect.id === 'sunnyday' || effect.id === 'desolateland') {
				this.damage(target.maxhp / 10, target, target);
			}
		},
		flags: {},
		name: "Solar Power",
	...
	windrider: {
		...
		onSideConditionStart(side, source, sideCondition) {
			const pokemon = this.effectState.target;
			if (sideCondition.id === 'tailwind' && pokemon?.isActive) {
				this.boost({atk: 1}, pokemon, pokemon);
			}
		},
		flags: { breakable: 1 },
