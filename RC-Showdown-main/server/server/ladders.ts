...
class Ladder extends LadderStore {
	...
	async searchBattle(user: User, connection: Connection) {
		console.log(`[SEARCH DEBUG] searchBattle called: user=${user.id}, format=${this.formatid}`);
		if (!user.connected) { console.log(`[SEARCH DEBUG] user not connected, aborting`); return; }

		const format = Dex.formats.get(this.formatid);
		console.log(`[SEARCH DEBUG] format: id=${format.id}, searchShow=${format.searchShow}, rated=${format.rated}, exists=${format.exists}`);
		if (!format.searchShow) {
			console.log(`[SEARCH DEBUG] format not ladderable (searchShow=false)`);
			connection.popup(`Error: Your format ${format.id} is not ladderable.`);
			return;
		}

		const oldUserid = user.id;
		const challengeType = format.rated ? 'rated' : 'unrated';
		const isRated = format.rated !== false;
		console.log(`[SEARCH DEBUG] calling prepBattle: challengeType=${challengeType}, isRated=${isRated}`);
		const search = await this.prepBattle(connection, challengeType, null, isRated);

		if (oldUserid !== user.id) { console.log(`[SEARCH DEBUG] user changed id during prep, aborting`); return; }
		if (!search) { console.log(`[SEARCH DEBUG] prepBattle returned null, aborting`); return; }

		console.log(`[SEARCH DEBUG] prepBattle succeeded, rating=${search.rating}, calling addSearch`);
	...
	matchmakingOK(matches: [BattleReady, User][]) {
		...
		if (new Set(users).size !== users.length) { console.log(`[MM DEBUG] same user, rejected`); return false; }

		if (Config.noipchecks) {
			console.log(`[MM DEBUG] noipchecks=true, accepting match`);
		...
		console.log(`[MM DEBUG] IP check: ${users.map(u => u.latestIp).join(' vs ')}`);
		if (new Set(users.map(user => user.latestIp)).size !== users.length) { console.log(`[MM DEBUG] same IP, rejected`); return false; }
	...
	addSearch(newSearch: BattleReady, user: User) {
		...
		console.log(`[SEARCH DEBUG] addSearch: user=${user.id}, format=${formatid}, rating=${newSearch.rating}`);
		...
		if (!formatTable) {
			...
			console.log(`[SEARCH DEBUG] created new formatTable, playerCount=${formatTable.playerCount}`);
		}
		console.log(`[SEARCH DEBUG] existing searches in queue: ${formatTable.searches.size} [${[...formatTable.searches.keys()].join(', ')}]`);
		if (formatTable.searches.has(user.id)) {
			console.log(`[SEARCH DEBUG] user already searching, aborting`);
		...
		for (const search of formatTable.searches.values()) {
			...
			if (!searcher) { console.log(`[SEARCH DEBUG] searcher for ${search.userid} not found, skipping`); continue; }
			console.log(`[SEARCH DEBUG] checking match: ${user.id}(${newSearch.rating}) vs ${searcher.id}(${search.rating})`);
			const matched = this.matchmakingOK([[search, searcher], [newSearch, user]]);
			console.log(`[SEARCH DEBUG] matchmakingOK result: ${matched}`);
			if (matched) {
				matches.push(search);
			}
			if (matches.length >= formatTable.playerCount) {
				console.log(`[SEARCH DEBUG] MATCH FOUND! Starting battle...`);
		...
		console.log(`[SEARCH DEBUG] no match found, adding ${user.id} to queue (now ${formatTable.searches.size + 1} in queue)`);
