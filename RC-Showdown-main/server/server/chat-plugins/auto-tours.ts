...
function patchTournamentEnd(tour: TournamentType, room: BasicRoom) {
	...
	tour.onTournamentEnd = function () {
		// Schedule the next tournament BEFORE the tournament is destroyed so state is valid
		scheduleNext(room);

		// Award tokens and show leaderboard
		try {
			const rawResults = tour.generator.getResults();
			if (Array.isArray(rawResults)) {
				const results = (rawResults as any[][]).map(
		...
		// Call the original onTournamentEnd (destroys the tournament)
		originalEnd();
	};
}
...
function createAutoTournament(room: BasicRoom) {
	...
	try {
		...
		if (tour) {
			...
			// and schedule the next tournament
		...
		room.update();
		advanceFormat(room);
		// Schedule next even on error so auto-tours keep running
		scheduleNext(room);
	}
}
